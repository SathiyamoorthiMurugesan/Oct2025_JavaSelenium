...
//so, we are assigning the method value to a variable 'myName', and we can use that to call the method.
 ...
 //methodE returns SecondClass. So we are assigning that methodE to a variable var1. 
 //so that we can access the methods from SecondClass through this var1.
        ...
        AAA interfaceVar = obj2.method2();  //method2 returns an interface AAA. so we can access methods in that interface through interfaceVar
...
interface AAA{
	void methodG();  //If we create body ({}) for this method, it throws error as 'abstract method don't have body'
